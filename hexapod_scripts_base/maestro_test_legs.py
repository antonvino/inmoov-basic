...
hexapod_legs_safe = [
    [[804,2204], [996,2108], [628,1500]],  # leg 1
    [[596,1900], [804,1900], [500,1548]],  # leg 2
    [[404,1804], [1284,2412], [756,1900]],  # leg 3
    [[1092,2348], [1092,2156], [996,2108]],  # leg 4
    [[756,2108], [596,1548], [708,1596]],  # leg 5
    [[1092,2508], [708,1708], [596,1500]],  # leg 6
]
...
# Take inputs and set servo targets accordingly
leg = int(sys.argv[1])-1  # I say "1" code takes 0
joint = int(sys.argv[2])-1
cmd = sys.argv[3]

srv = leg*3+joint
pos = 1500
pos = servo.get_position(srv)
...
if pos is not None:
    if (cmd == "+" or cmd == "-"):
        ...
        try:
            while 1:
                pos+=delta
                print 'new pos: {0}'.format(pos)
                servo.set_target(srv,pos)
                time.sleep(0.05)
        ...
        pos = None
        if(go == "high"):
             pos = hexapod_legs_safe[leg][joint][1]
        elif(go == "low"):
             pos = hexapod_legs_safe[leg][joint][0]
        elif(go == "middle"):
             pos = hexapod_legs_safe[leg][joint][0] + int(floor((hexapod_legs_safe[leg][joint][1]-hexapod_legs_safe[leg][joint][0])/2))
        print 'new pos: {0}'.format(pos)
        servo.set_target(srv,pos)
